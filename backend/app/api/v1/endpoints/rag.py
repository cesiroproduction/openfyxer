        ...
        for doc in docs:
            sources.append(
                RAGSource(
                    ...
                    id=doc.id,
                    title=doc.filename,
                    snippet=doc.content_summary or doc.content_text[:200] if doc.content_text else "",
        ...
        for meeting in meetings:
            sources.append(
                RAGSource(
                    ...
                    id=meeting.id,
                    title=meeting.title,
                    snippet=meeting.summary or meeting.transcript[:200] if meeting.transcript else "",
                    relevance_score=0.7,
...
async def upload_document(
    ...
    if file.content_type not in allowed_types:
        raise HTTPException(
            ...
            detail=f"File type not allowed. Allowed types: PDF, DOCX, TXT",
        )
...
async def query_knowledge_graph(
    node_type: Optional[str] = Query(None, pattern="^(Person|Company|Project|Email|Document|Meeting|Topic)$"),
